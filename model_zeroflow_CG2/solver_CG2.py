...
class Solver():
    ...
    def setup_function_spaces_ODE(self):
        ...
        self.S = VectorFunctionSpace(self.mesh, "CG", 2, dim=dim)

        inits_ODE = self.problem.inits_ODE
        self.ss_ = interpolate(inits_ODE, self.S)
    ...
    def setup_function_spaces_PDE(self):
        ...
        # define function space
        DG0 = FiniteElement('DG', self.mesh.ufl_cell(), 1)    # DG0 element
        CG1 = FiniteElement('CG', self.mesh.ufl_cell(), 2)    # CG1 element
    ...
    def ODE_solver(self):
        ...
        A_ODE = 1.0/self.dt_ODE*inner(self.ss-self.ss_,qs)*dx - F_exprs_q*dx

        # initiate solver
        J_ODE = derivative(A_ODE, self.ss)                                   # calculate Jacobian
        problem_ODE = NonlinearVariationalProblem(A_ODE, self.ss, [], J_ODE) # create problem
        self.solver_ODE  = NonlinearVariationalSolver(problem_ODE)          # create solver
        prm_BE = self.solver_ODE.parameters                                # get parameters
    ...
    def solve_system_godunov(self, path_results=False):
        ...
        if self.N_states > 0:
            self.solver_ODE.solve()
            self.ss_.assign(self.ss)
