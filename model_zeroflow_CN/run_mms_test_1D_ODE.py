...
def space_time():
    ...
    title_fsum_L2 = directory + "/convergence_table_summary_L2_st.txt"
    title_fsum_H1 = directory + "/convergence_table_summary_H1_st.txt"
    ...
    fsum_L2 = open(title_fsum_L2, 'w+')
    fsum_H1 = open(title_fsum_H1, 'w+')
    ...
    for resolution in resolutions:
        ...
        # time variables
        dt_value = dt_0/(2**i)              # time step
        t = Constant(0.0)                   # time constant
        t_prev = Constant(0.0)              # time constant

        problem = ProblemMMS(mesh, boundary_point, t, t_prev)
        # solve system
        ...
        if i == 0:
            # write to file - summary L2
            fsum_L2.write('%g & %.2E(-----) & %.2E(-----) & %.2E(-----) & \
                    %.2E(-----)\\\\' % (N, NaE_L2, phiN_L2, alphaN_L2, m_L2))

            # write to file - summary H1
        ...
        if i > 0:
            ...
            f1.write('%g & %.2E & %.2E & %.2E(%.2f) \\\\' % (N, h, dt_value, alphaN_L2, r_alphaN_L2))
            # write to file - L2/H1 err and rate - Na
            f2.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N, h,dt_value,\
                            NaN_L2, r_NaN_L2, NaE_L2, r_NaE_L2))
            # write to file - L2/H1 err and rate - K
            f3.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N,h,dt_value,\
                            KN_L2, r_KN_L2, KE_L2, r_KE_L2))
            # write to file - L2/H1 err and rate - Cl
            f4.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N,h,dt_value,\
                            ClN_L2, r_ClN_L2, ClE_L2, r_ClE_L2))
             # write to file - L2/H1 err and rate - Cl
            f5.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N,h,dt_value,\
                            phiN_L2, r_phiN_L2, phiE_L2, r_phiE_L2))

            # write to file - L2/H1 err and rate - Na
            f7.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N,h,dt_value,\
                            NaN_H1, r_NaN_H1, NaE_H1, r_NaE_H1))
            # write to file - L2/H1 err and rate - K
            f8.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N,h,dt_value,\
                            KN_H1, r_KN_H1, KE_H1, r_KE_H1))
            # write to file - L2/H1 err and rate - Cl
            f9.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N,h,dt_value,\
                            ClN_H1, r_ClN_H1, ClE_H1, r_ClE_H1))
             # write to file - L2/H1 err and rate - Cl
            f10.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) \\\\' % (N,h,dt_value,\
                            phiN_H1, r_phiN_H1, phiE_H1, r_phiE_H1))

            # write to file - L2/H1 err and rate - Cl
            f11.write('%g & %.2E & %.2E &%.2E(%.2f) & %.2E(%.2f) & %.2E(%.2f)\\\\' % (N,h,dt_value,\
...
def space():
    ...
    title_fsum_L2 = directory + "/convergence_table_summary_L2_s.txt"
    title_fsum_H1 = directory + "/convergence_table_summary_H1_s.txt"
...
def time():
    ...
    title_fsum_L2 = directory + "/convergence_table_summary_L2_t.txt"
    title_fsum_H1 = directory + "/convergence_table_summary_H1_t.txt"
    ...
    # create mesh
    N = 2**11                           # number of cells
    mesh = IntervalMesh(N, 0, 1)        # mesh
    h = mesh.hmin()                     # minimum diameter of cells
    boundary_point = "near(x[0], 0.0)"

    # baseline time step
    dts = [8.0e-2, 4.0e-2, 2.0e-2, 1.0e-2, 5.0e-3, 2.5e-3, 1.25e-3]
    Tstop = 2*dts[0]
