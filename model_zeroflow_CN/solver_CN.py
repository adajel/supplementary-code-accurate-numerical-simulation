...
class Solver():
    ...
    def setup_function_spaces_PDE(self):
        ...
        self.w = project(inits_PDE, self.W)
        return
    ...
    def ODE_solver(self):
        ...
        if self.MMS_test:
            src_ts_ODE = self.problem.source_terms_ODE
            F_exprs = F(self.w_, s, src_ts_ODE, self.problem.t_ODE)
        else:
        ...
        # create ODE scheme (NB! Need explicit scheme, e.g. RK4)
        Scheme = eval("RK4")
    ...
    def PDE_solver(self):
        ...
        J_M = self.problem.get_membrane_fluxes(self.w, self.ss)
        J_M_ = self.problem.get_membrane_fluxes(self.w_, self.ss)
        ...
        phi_E_ = ww_[N_comparts*(2 + N_ions) - 2] # ECS potential previous
        v_phi_E = vv[N_comparts*(2 + N_ions) - 2] # ECS potential test function
        a_E = a[N_comparts - 1]                   # immobile ions ECS
        ...
        for j in range(N_comparts - 1):
            # shorthands
            phi_I = ww[N_comparts*(1 + N_ions) - 1 + j]   # ICS potential unknown
            phi_I_ = ww_[N_comparts*(1 + N_ions) - 1 + j] # ICS potential previous
            ...
            # initiate previous transmembrane water flux (m/s)
            w_M_ = a_E/alpha_E_ - a_I/alpha_I_
            ...
            for i in range(N_ions):
                ...
                # previous compartmental ion flux for ion i in compartment j - (mol/m^2s)
                J_I_ = - D_ij*(grad(k_I_) + z[i]*F*k_I_/(R*temperature)*grad(phi_I_))

                # form for conservation of ion i in compartment j - (mol/m^3s)
                A_k_I += 1.0/self.dt*inner(alpha_I*k_I - alpha_I_*k_I_, v_k_I)*dx \
                       - 0.5*inner(J_I + J_I_, grad(v_k_I))*dx \
                       + 0.5*gamma_M[j]*inner(J_M[i][j] + J_M_[i][j], v_k_I)*dx

                # form for conservation of ion i in ECS - (mol/m^3s)
                A_k_E += - 0.5*gamma_M[j]*inner(J_M[i][j] + J_M_[i][j], v_k_E)*dx
            ...
            A_alpha_I += 1.0/self.dt*inner(alpha_I - alpha_I_, v_alpha_I)*dx \
                       + 0.5*gamma_M[j]*nw_M[j]*R*temperature*inner(w_M + w_M_, v_alpha_I)*dx
        ...
        for i in range(N_ions):
            ...
            # previous compartmental ion flux for ion i in compartment j - (mol/m^2s)
            J_E_ = - D_iE*(grad(k_E_) + z[i]*F*k_E_/(R*temperature)*grad(phi_E_))

            # form for conservation of ion i in ECS - (mol/m^3s)
            A_k_E += 1.0/self.dt*inner(alpha_E*k_E - alpha_E_*k_E_, v_k_E)*dx \
                   - 0.5*inner(J_E + J_E_, grad(v_k_E))*dx

            # add ion specific part to form for ECS potential (C/m^3)
            A_phi_E += - F*alpha_E*z[i]*k_E*v_phi_E*dx

        # Add source and boundary terms for MMS (if MMS)
        A_MMS = 0
        # check if problem is a test problem (MMS test)
        if self.MMS_test:
            # get source and boundary terms
            src_ts_PDE = self.problem.source_terms_PDE
            src_ts_PDE_ = self.problem.source_terms_PDE_
            bndr_ts = self.problem.boundary_terms
            bndr_ts_ = self.problem.boundary_terms_
            ...
            for i in range(N_comparts*(2 + N_ions) - 1):
                A_MMS += - 0.5*inner(src_ts_PDE[i] + src_ts_PDE_[i], vv[i])*dx
                if bndr_ts[i] is not None:
                    if len(n) == 1:
                        # add boundary terms for 1D case
                        A_MMS += 0.5*inner(bndr_ts[i] + bndr_ts_[i], vv[i])*ds
                    else:
                        # add boundary terms for ND, N > 1 case
                        A_MMS += 0.5*inner(dot(bndr_ts[i] + bndr_ts_[i] , n), vv[i])*ds

        # gather variational formulation
        ...
        PDE_solver  = NonlinearVariationalSolver(problem)        # create solver
        prm = PDE_solver.parameters                              # get parameters
    ...
    def solve_system_strang(self, path_results=False):
        ...
        and s^{n-1} = [s1^{n-1} , ... , s5^{n-1}] are known.
            ...
            (1) Update w^n by solving PDEs, with s^{n-1} from ODE step,
                and set w^{n-1} = w^n
            (2) Update s^n by solving ODEs, with phi_M^{n} from PDE step,
                and set s^{n-1] = s^n
        ...
        while (float(self.problem.t_PDE) <= self.Tstop):
            ...
            # solve tentative ODE step (and NB! update internal time ODE solver)
            if self.N_states > 0:
                self.pi_solver.step(float(0.5*self.dt))

            # update internal time PDE solver
            self.problem.t_PDE.assign(float(self.problem.t_PDE + self.dt))

            if self.MMS_test:
                self.problem.t_PDE_prev.assign(float(self.problem.t_PDE - self.dt))

            # solve PDE system
            self.PDE_solver.solve()
            # update previous PDE solutions
    ...
    def initialize_xdmf_savefile_ODE(self, file_prefix):
        ...
        for idx in range(N_unknows_ODE):
            ...
            xdmf_file.write(self.ss.split()[idx], self.problem.t_PDE.values()[0])
        return
    ...
    def save_xdmf_ODE(self):
        ...
        for idx in range(len(self.xdmf_files_ODE)):
            self.xdmf_files_ODE[idx].write(self.ss.split()[idx], self.problem.t_PDE.values()[0])
        return
